...
Begin VB.Form Cesiond 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cesión de derechos onerosa"
   ClientHeight    =   6615
   ClientLeft      =   150
   ClientTop       =   435
   ClientWidth     =   3615
   Icon            =   "Copia de Form1.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6615
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CommandButton Command1 
         ...
         Height          =   495
         Left            =   120
         Picture         =   "Copia de Form1.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   17
...
arAncel = AgregaDecimales(CStr(CDbl(VEconomico) * 0.02 + 27.7) * 0.8)
certificado = AgregaDecimales(CStr(700))
aportes1 = AgregaDecimales(CStr((((CDbl(VEconomico) * 0.02 + 27.7) * 0.8) / 2) * 0.18))
aportes2 = AgregaDecimales(CStr(CDbl(VEconomico) * 0.003) * 0.8)
reposicion = AgregaDecimales(CStr(CDbl(VEconomico) * 0.01))
'MsgBox "El minimo de la rep es el mismo????"
If reposicion < 30 Then
reposicion = "30,00"
End If
anotacion = AgregaDecimales(CStr(0))
...
'pWt = GenHTML("Escribanía " & GetSetting("Presupuestos", "Escribania", "Nombre", "Ventura"), GetSetting("Presupuestos", "Escribania", "Direccion", "Corrientes 5 - 4º " & Chr(34) & "A" & Chr(34)), GetSetting("Presupuestos", "Escribania", "Telefonoemail", "Te: 4255715 - e-mail: gabventura@arnet.com.ar"), partes, VEconomico, arAncel, certificado, aportes1, aportes2, reposicion, anotacion, goperativo, total, notros1, otros1, notros2, otros2, notros3, otros3)
...
Private Sub reposicion_GotFocus()
reposicion.SelStart = 0
reposicion.SelLength = Len(reposicion)
End Sub

Private Sub reposicion_LostFocus()
If IsNumeric(reposicion) = False Then
reposicion = "0,00"
Else
total = SumarTodo()
reposicion = AgregaDecimales(CStr(reposicion))
End If
End Sub
...
SumarTodo = AgregaDecimales(CStr(CDbl(arAncel) + CDbl(certificado) + CDbl(aportes1) + CDbl(aportes2) + CDbl(reposicion) + CDbl(anotacion) + CDbl(goperativo) + CDbl(ProtoLey) + CDbl(otros1) + CDbl(otros2) + CDbl(otros3)))
End Function
